...
async def run_ks_to_gate2(orch, project_id, duration_seconds):
    """Drive orchestrator through KS briefing rounds to gate2."""
    ...
    await orch.process_event(project_id, "generate_content_spine", {
        "point_of_view": "Practical perspective"
    })
    ...
    # brief_approve auto-runs Director → outline → gate2
    r = await orch.process_event(project_id, "brief_approve", {})
    assert r["success"] is True, f"brief_approve failed: {r.get('error')}"
    return r
