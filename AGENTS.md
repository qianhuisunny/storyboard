...
| `backend/app/services/state.py` | State machine + DB-backed pipeline state (`pipeline_states.state_data`) |
...
| `data/` | Raw project artifacts only (uploads, fetched links, exports, `data/example/` fixtures). Project metadata, pipeline state, stage snapshots, and upload metadata live in SQLite. |
...
- **Data schema changes** — any change to project/story schema must be backwards-compatible with rows already persisted in SQLite and any raw exports under `data/`
...
- **Treat SQLite as the source of truth** — project metadata, pipeline state, stage autosaves, and upload metadata must go through SQLAlchemy, not `data/project_{id}/*.json`
- **Test DB-backed flows first** — use `data/example/` for fixture payloads/parser tests, not as a replacement persistence path
...
| `POST` | `/api/project/{id}/event` | Process state machine event (`submit`, `approve`, `edit`, `refine_outline`, etc.) |
...
StoryboardDirector   → determines scene structure and flow
      ↓ outline
      [QualityGate]  → validates outline structure + one holistic review, retries generation once
      ↓ graded outline
StoryboardWriter     → writes detailed screen-by-screen content (includes duration calculation)
      ↓ storyboard
      [QualityGate]  → validates storyboard structure + one holistic review with outline context
      ↓ final_storyboard
```
...
6. **Data layer is SQLite**: `projects`, `pipeline_states`, `stage_snapshots`, and `uploads` are the source of truth. Do not add or rely on `data/project_{id}/*.json` fallbacks for project metadata or pipeline/stage state. The filesystem is only for raw artifacts such as uploads, fetched links, and exported story files.
...
### 2026-03-23: Prompt iteration and code iteration must be decoupled

**Problem:** Content spine generation prompt was an inline f-string in `brief_builder.py`. Changing prompt wording required editing Python, risking syntax errors, and restarting the backend. Prompt iteration speed was bottlenecked by code deployment.

**Lesson:**
- Every LLM instruction belongs in a `prompts/*.md` file, loaded via the agent's `prompt_file` attribute. Code only provides contextual data (brief fields, user input) as the user prompt.
- If an agent has a `prompt_file`, that IS its system prompt. Don't bypass it with `system_prompt_override` or custom loaders — that breaks the pattern other agents follow.
- Corollary: if a prompt file isn't being loaded by any agent, it's dead — archive it immediately.

---

### 2026-03-23: Dead code is immediate — not "later"

**Problem:** `BRIEF_BUILDER_SYSTEM_PROMPT.md` was loaded by BaseAgent but never reached an LLM call (Round 1/2 don't use LLM, Round 3 used a different prompt via override). `must_avoid` was generated by the LLM, parsed, stored, but hidden from the UI and unused by downstream agents.

**Lesson:**
- When functionality is disabled or replaced, remove the code in the same change. Don't leave commented-out code, orphaned files, or half-alive fields "in case we need it later." Create a task instead.
- The test: if you `grep` for it and find no live consumer, it's dead. Archive/delete + open a task if there's future intent.

---

### 2026-03-23: State must survive refresh — write back on every transition

**Problem:** User confirmed Section 1, backend stored values in `state.confirmed_fields` but NOT in `state.story_brief["fields"]`. Hard refresh → frontend reads `story_brief.fields` from pipeline-state → gets original empty values → user's input is lost.

**Lesson:**
- Every event handler that accepts user input (`confirmed_fields`, edits, selections) must write the values back to `state.story_brief["fields"]` — not just to `state.confirmed_fields`.
- Pattern: `state.story_brief["fields"] = {**existing, **confirmed_fields}` in every round confirm handler.
- When fixing one handler, audit all sibling handlers for the same pattern. A bug in `round1_confirm` is almost certainly also in `round2_confirm` and `round3_confirm`.

---

### 2026-03-23: Fix one pattern bug, audit all siblings

**Problem:** `round1_confirm` didn't write confirmed values back to `story_brief["fields"]`. Fixed it — then found `round2_confirm` and `round3_confirm` had the exact same bug.

**Lesson:**
- A pattern bug is never in just one place. When you fix a handler/component, immediately grep for all siblings that follow the same pattern and check each one.
- This applies to: event handlers, round confirm functions, stage components, prompt-agent mappings.
