...
Duration
{Target seconds as integer, e.g. "90"}
...
- The brief's total duration (in seconds) is your **budget**. You cannot exceed it.
- Allocate a **single target duration in seconds** per section. Not a range — one number.
- **Sum check**: Add up all your target durations. The total MUST equal the brief's total duration exactly. If it doesn't, redistribute until it does.
- Hook/closing: typically 15–90 seconds. Body sections: typically 30–150 seconds.
- A 5-minute (300s) video should NOT have any single section longer than 120 seconds.
- A 10-minute (600s) video should NOT have any single section longer than 180 seconds.
...
- [ ] Duration targets sum to exactly the brief's total duration
