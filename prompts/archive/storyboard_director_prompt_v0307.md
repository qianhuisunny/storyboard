...
```json
{
  ...
  "ALLOWED_SCREEN_TYPES": ["stock video", "screencast", "slides/text overlay", "CTA"]
}
```
...
| Type | When to use |
|------|-------------|
| `stock video` | Emotional context, real-world scenarios, hooks |
| `screencast` | Product UI, step-by-step workflows |
| `slides/text overlay` | Statistics, lists, abstract concepts |
| `talking head` | Credibility moments (only if allowed) |
...
```json
[
  { "screen_number": 1, "screen_type": "stock video", "voiceover_text": "...", "target_duration_sec": 6.5 },
  { "screen_number": 2, "screen_type": "slides/text overlay", "voiceover_text": "...", "target_duration_sec": 7.0 }
]
```
